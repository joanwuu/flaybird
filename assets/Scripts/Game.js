...
var Game = cc.Class({
  ...
  onLoad: function () {
    ...
    if (Storage.getHighScore() > 0) {
      this.highScoreText.string =
        Constant.HIGHSCORE_TXT + Storage.getHighScore()
    }
  },
  ...
  spawnPipes: function () {
    ...
    var pipeGap =
      Math.floor(Math.random() * (this.pipeMaxGap - this.pipeMinGap)) +
      this.pipeMinGap
  ...
  gameUpdate: function () {
    for (var i = 0; i < this.pipes.length; i++) {
      ...
      if (
        curPipeNode.x < -(this.size.width / 2 + Constant.PIPE_SPAWN_OFFSET_X)
      ) {
